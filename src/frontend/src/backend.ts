...
export class Backend implements backendInterface {
    ...
    async createPayment(arg0: PaymentRequest): Promise<void> {
        if (this.processError) {
            try {
...
function to_candid_variant_n7(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: TuitionType): {
    ...
    return value == TuitionType.both ? {
        both: null
    } : value == TuitionType.offline ? {
        offline: null
    } : value == TuitionType.online ? {
        online: null
    } : value;
}
